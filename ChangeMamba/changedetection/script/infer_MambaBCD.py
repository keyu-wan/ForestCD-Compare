import sys
sys.path.append('/mnt/project-data/kwan/AI_Forestry/model/MambaCD')
...
class Inference(object):
    ...
    def infer(self):
        ...
        kc = self.evaluator.Kappa_coefficient()
                # ====== 追加：从混淆矩阵计算 per-class 与 macro ======
        cm = self.evaluator.confusion_matrix.astype(np.float64)
        eps = 1e-7
        tp = np.diag(cm)
        fp = cm.sum(axis=0) - tp
        fn = cm.sum(axis=1) - tp

        precision_c = tp / (tp + fp + eps)           # 每类 Precision
        recall_c    = tp / (tp + fn + eps)           # 每类 Recall（= per-class accuracy）
        f1_c        = 2 * precision_c * recall_c / (precision_c + recall_c + eps)
        iou_c       = tp / (tp + fp + fn + eps)

        precision_mean = np.mean(precision_c)
        recall_mean    = np.mean(recall_c)
        f1_mean        = np.mean(f1_c)
        iou_mean       = np.mean(iou_c)

        #（可选）原实现里的 per-class accuracy（= recall）也可以直接调用现有函数
        mAcc, Acc_per_class = self.evaluator.Pixel_Accuracy_Class()

        # 友好打印：同一指标 -> mean / class0 / class1
        def fmt3(name, mean, c):
            if len(c) >= 2:
                print(f"{name}: mean={mean:.6f}, c0={c[0]:.6f}, c1={c[1]:.6f}")
            else:
                # 非二分类时就打印所有类
                print(f"{name}: mean={mean:.6f}, per_class={c.tolist()}")

        fmt3("Precision", precision_mean, precision_c)
        fmt3("Recall",    recall_mean,    recall_c)
        fmt3("F1",        f1_mean,        f1_c)
        fmt3("IoU",       iou_mean,       iou_c)

        fmt3("Per-class Acc (==Recall)", mAcc, Acc_per_class)
